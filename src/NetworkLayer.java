...
public class NetworkLayer implements Layer {
  ...
  private static final int MAX_VECTOR_COUNT = 6; // send a VECTOR packet every
                                                 // MAX_VECTOR_COUNT*VECTOR_DELAY
                                                 // milliseconds, even there is
                                                 // no change
  ...
  // ////////////////////////////////////////////////////////////////////////
  // Definitions of the DV policy
  // You can change initial values
  // These flags may be also updated through the control feature. You have to use
  // them in your DV implementation.
  ...
  /**
   * Initializes this {@code NetworkLayer} and binds it to the specified name,
   ...
   * housekeeping tasks for the network protocol.
   */
  public void start() {
    ...
    TIMER.schedule(new TimerTask() {
      ...
      public void run() {
        ...
        try {
          ...
          if (!removed.isEmpty()) {
            boolean updated = false; // a flag that indicates if there is a
                                     // change or not in DV
            System.out.println("removing devices " + removed);
            for (String neighbor : removed) {
              linkState.remove(neighbor);
              // it should also be handled according to the distance vector
              // protocol
              // ...
            }
  ...
  /**
   * Builds a {@code VECTOR} packet, ready to be sent. The destination is given
   * as a parameter. When {@code destination} is {@code null}, the packet is
   * intended to be sent to every direct neighbor. When {@code destination} is
   * not {@code null}, a specific packet can be built for this destination.
   * 
   * @param destination
   *          the intended destination for this packet, or {@code null} to
   *          specify any direct neighbor as the destination
   * @param cause
   *          a {@code String} indicating why this packet will be sent
   ...
   */
  private Packet makeVectorPacket(String destination, String cause,
      boolean incremental) {
    String[][] vector = forwardingTable.makeVector(destination,
        poisonedReverse, true, incremental);
    ...
    System.out.println("====== " + myName + " SENDING to " + packetDestination
        + " (" + cause + ") ==========");
    printVector(vector);
    if (vector.length > 0)
      return new VectorPacket(myName, packetDestination, vector);
    return null;
  }

  /**
   * The basic method used to send a vector. Always call this. It may be then
   * easily patched to implement variants in the Distance Vector algorithm.
   * 
   * @param cause
   *          a {@code String} indicating why this packet will be sent
   *          (for debugging purpose)
   * @param incremental
   *          indicating whether the routing table is sent completely or not
   *          (i.e. recent changes only)
   */
  private void sendVector(String cause, boolean incremental) {
    // TO BE COMPLETED
    forwardingTable.clearUpdated(); // here is a good place
  }

  /**
   * Adjusts the content of the neighborhood table, after the receipt a
   * {@code HELLO} or {@code VECTOR} packet from a given source.
   * 
  ...
  private void handleSendingNeighbor(String name, Link from) {
    ...
    if (link == null) {
      ...
      // in NetWorkLayer this event will also be handled according to the
   ...
   */
  private void handleVector(String source, String destination,
      String[][] vector, Link from) {
    if (source.equals(myName))
      return; // don't consider my own packets
    if (!destination.equals(myName) && !destination.equals(ALL))
      return; // it may be a poisoned vector !
    boolean updated = false; // a flag that indicates if it will result in a
                             // change in DV
    // TO BE COMPLETED
    // also imagine the case this vector arrives before an HELLO
  }
  ...
  private void handleNextHop(int ttl, Packet payload) {
    if (payload.getDestination().equals(myName)) {
      if (aboveLayer != null)
        aboveLayer.receive(payload, this);
    } else
      forwardingTable.forward(payload, ttl - 1);
  }
  ...
  public void receive(Packet packet, Layer from) {
    ...
    try {
      ...
      case HELLO:
        ...
        return;
      case VECTOR:
        handleVector(packet.getSource(), packet.getDestination(),
            packet.getArray(), link);
        return;
      case NEXTHOP:
        if (packet.getDestination().equals(myName))
          handleNextHop(packet.getNum(), packet.getNestedPacket());
        return;
      default:
  ...
  public void send(Packet packet) {
    if (packet.getDestination().equals(myName)) { // loopback
      if (aboveLayer != null)
        aboveLayer.receive(packet, this);
    } else
      forwardingTable.forward(packet, RouteEntry.INFINITY - 1); // max TTL value
  }
