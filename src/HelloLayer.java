...
public class HelloLayer implements Layer {
	 ...
	 * which is supposed to be unique. That is to say, gives it an identity on
	 * the network. The specified name will be used as the source field of
	 * outgoing packets and for filtering incoming packets on their destination
	 * field. Also performs various initializations.
	 * 
	 * @param name
	 ...
	 * the incoming packets that have to be processed at above layers. Thus,
	 * this {@code NetworkLayer} will invoke the {@link #receive receive} method
	 * of the specified {@code above} layer to pass it a packet upward.
	 * 
	 * @param above
	 *            the {@code Layer} whose {@link #receive receive} method must
	 *            be called to pass it an incoming packet
	 */
	...
	public void start() { // TO BE COMPLETED
		RegularSendingTask task = new RegularSendingTask(underLayers, lock,
				HELLO, linkState);
		TIMER.schedule(task, 0, HELLO_DELAY);
	}
	 ...
	 * table. Otherwise, but only when the link is unchanged, the down-counter
	 * is reset to the maximal value.
	 * 
	 * @param name
	 ...
	 * every incoming packet to this {@code NetworkLayer}. This method should
	 * not block long and must return as soon as possible.
	 * 
	 * @param packet
	 *            the incoming packet
	 * @param from
...
class RegularSendingTask extends TimerTask {
	private Set<Link> underLayers;
	private Lock externalLock;
	private final Packet HELLO;
	private LinkState linkState;

	public RegularSendingTask(Set<Link> ul, Lock eu, Packet H, LinkState ls) {
		this.underLayers = ul;
		this.externalLock = eu;
		this.HELLO = H;
		this.linkState = ls;
	}
