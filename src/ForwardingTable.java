...
public class ForwardingTable {
	 ...
	 * is specified as a {@code RouteEntry}. The destination field of the entry
	 * is a key in the table, hence if the table previously contained an entry
	 * for the same destination, then the former entry is replaced by the
	 * specified value, otherwise the specified value is added as a new entry.
	 * 
	...
	/**
	 * Indicates whether the specified destination is currently marked as
	 * updated in this {@code ForwardingTable}.
	 * 
	 * @param destination
	...
	/**
	 * Whatever their state before this call, all destinations registered in
	 * this {@code ForwardingTable} are no longer marked as updated.
	 */
	...
	/**
	 * Dumps the whole content of this {@code ForwardingTable} onto the
	 * specified {@code PrintStream}. The display order is not specified.
	 * 
	 * @param out
	 ...
	 * enabled and the produced vector also depends on the
	 * {@code poisonedReverse} parameter.
	 * 
	 * If the {@code addLost} parameter is {@code false}, the returned vector
	 * does never contain any destination registered in this
	 * {@code ForwardingTable} with RouteEntry.INFINITY as metrics. This should
	 * not be confused with an infinite metrics forced by the "poisoned-reverse"
	 * rule.
	 * 
	 ...
	 * with RouteEntry.INFINITY as metrics, according to the two last rules
	 * below. This allows "route poisoning".
	 * 
	 ...
	 * 
	 * If the {@code incremental} parameter is {@code false}, the returned
	 * vector contains all the destinations satisfying the above rules. This may
	 * be used for periodically sending a 'full' vector or for feeding a newly
	 * discovered neighbor.
	 * 
	 ...
	 * 
	 * @param recipient
	 *            the specific destination for this packet, or {@code null} if
	 *            any direct neighbor will be the destination
	 * @param poisonedReverse
	 *            indicating whether the "poisoned-reverse" variant is applied
	 *            in addition to the "split-horizon" rule
	 * @param addLost
	...
	/**
	 * Forwards the given payload packet with the specified (already
	 * decremented) TTL. The packet is discarded in any of the following cases:
	 * 
